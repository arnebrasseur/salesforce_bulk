...
==This branch (http://github.com/arnebrasseur/salesforce_bulk)

Differs considerably from the upstream version. It is more OO (separate objects with single responsibilities), HTTP requests and parsing of the response is handled by HTTParty, the code has been made more idiomatic Ruby. Less class variables should make the library easier to use in a multi-account setup.

A start has been made for unit testing with RSpec, something that was absent from the original version.
